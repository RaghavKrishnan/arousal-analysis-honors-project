...
% ----------------------------------------------
% Loading Emo-FilM BOLD timeseries
load('BetweenViewings_Cohort_3D_Master.mat'); 

% Permute axes from Python (ROIs, Time, Subj) to MATLAB (Subj, ROIs, Time)
BOLD = permute(master_array, [3, 1, 2]);

% Hyperparameters for tapered sliding window
wsize = [45];
sigma = 3;
% -----------------------------------------------
...
for ws = 1:length(wsize)
    ...
    % NSG saving 
    % Saves directly to the NSG execution folder to avoid directory path errors
    save(['sliding-dynFeat-', story, '.mat'], 'dynFeat');
    % -----------------------------------------
