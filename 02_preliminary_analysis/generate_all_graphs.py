...
OUTPUT_DIR     = '../results/all_movies_graphs/'
...
def process_sc_for_movie(movie_name):
    ...
    return np.mean(np.array([sc[:min_len] for sc in all_sc]), axis=0)
...
def load_intense_emotion(movie_name):
    ...
    if not os.path.exists(tsv_path) or not os.path.exists(json_path):
        print(f"    WARNING: Annotation file not found for {movie_name}")
        return None
...
def generate_dual_axis_plot(movie_name, emotion_data, sc_data, r_val, p_val):
    ...
    ax1.plot(time_sec, emotion_data, color=color_emotion, linewidth=2)
    ax1.tick_params(axis='y', labelcolor=color_emotion)

    ax2 = ax1.twinx()
    color_sc = 'tab:blue'
    ax2.set_ylabel('Mean Skin Conductance (μS)', color=color_sc, fontsize=12)
    ax2.plot(time_sec, sc_data, color=color_sc, linewidth=2)
    ax2.tick_params(axis='y', labelcolor=color_sc)

    p_str = "< 0.001" if p_val < 0.001 else f"{p_val:.3f}"
    plt.title(
        f'{movie_name}: Intense Emotion vs. Mean Skin Conductance  '
        f'(r = {r_val:.3f}, p {p_str})',
        fontsize=13, pad=12
    )

    fig.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f"{movie_name}_dual_axis.png"),
...
def main():
    ...
    for movie in movie_folders:
        ...
        if sc_signal is None or emotion_signal is None:
            print(f"  -> SKIPPED.")
            continue
